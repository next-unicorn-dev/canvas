...
const ChatTextarea: React.FC<ChatTextareaProps> = ({
  ...
  const [useBrandInfo, setUseBrandInfo] = useState(false)

  const imageInputRef = useRef<HTMLInputElement>(null)

  // Format brand info for system prompt
  const formatBrandInfoForPrompt = (brandInfo: BrandInfo): string => {
    const parts: string[] = []
    if (brandInfo.name) parts.push(`Brand Name: ${brandInfo.name}`)
    if (brandInfo.description) parts.push(`Description: ${brandInfo.description}`)
    if (brandInfo.industry) parts.push(`Industry: ${brandInfo.industry}`)
    if (brandInfo.targetAudience) parts.push(`Target Audience: ${brandInfo.targetAudience}`)
    if (brandInfo.brandColors) parts.push(`Brand Colors: ${brandInfo.brandColors}`)
    if (brandInfo.brandValues) parts.push(`Brand Values: ${brandInfo.brandValues}`)
    if (brandInfo.website) parts.push(`Website: ${brandInfo.website}`)
    if (brandInfo.socialMedia) parts.push(`Social Media: ${brandInfo.socialMedia}`)
    return parts.join('\n')
  }
  ...
  const handleSendPrompt = useCallback(async () => {
    ...
    // Get brand info if enabled
    let systemPrompt: string | undefined
    if (useBrandInfo) {
      try {
        const brandInfo = await getBrandInfo()
        console.log('📦 Brand info loaded:', brandInfo)
        const brandInfoText = formatBrandInfoForPrompt(brandInfo)
        console.log('📝 Formatted brand info:', brandInfoText)
        
        if (!brandInfoText || brandInfoText.trim() === '') {
          toast.warning(t('chat:textarea.noBrandInfo', 'No brand information found. Please set up your brand information in "My Brand Analysis" first.'))
          // Continue without brand info
        } else {
          const defaultSystemPrompt = localStorage.getItem('system_prompt') || DEFAULT_SYSTEM_PROMPT
          systemPrompt = `${defaultSystemPrompt}\n\n## Brand Information\n${brandInfoText}`
          console.log('✅ System prompt with brand info (first 500 chars):', systemPrompt.substring(0, 500))
          console.log('✅ Full system prompt length:', systemPrompt.length)
          console.log('✅ Brand info section:', `\n\n## Brand Information\n${brandInfoText}`)
        }
      } catch (error) {
        console.error('❌ Failed to get brand info:', error)
        toast.error(t('chat:textarea.brandInfoError', 'Failed to load brand information'))
      }
    }
      ...
      <div className="flex items-center justify-between gap-2 w-full">
        <div className="flex items-center gap-2 max-w-[calc(100%-50px)] flex-wrap">
          ...
          <div className="flex items-center gap-2 px-2">
            <Briefcase className={`size-4 ${useBrandInfo ? 'text-primary' : 'text-muted-foreground'}`} />
            <Switch
              checked={useBrandInfo}
              onCheckedChange={setUseBrandInfo}
              size="sm"
            />
            <span className={`text-xs whitespace-nowrap ${useBrandInfo ? 'text-primary font-medium' : 'text-muted-foreground'}`}>
              {t('chat:textarea.useBrandInfo', '내 브랜드')}
            </span>
          </div>
        </div>
