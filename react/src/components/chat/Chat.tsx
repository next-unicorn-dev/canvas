...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
          ...
          prev.forEach((msg) => {
            if (msg.role === 'assistant' && msg.tool_calls) {
              msg.tool_calls.forEach((tc) => {
                if (tc.id === data.id) {
                  // Add cancelled status marker
                  tc.result = 'TOOL_CALL_CANCELLED'
                }
              })
            }
          })
  ...
  const onSendMessages = useCallback(
    (data: Message[], configs: { textModel: Model; toolList: ToolInfo[]; systemPrompt?: string }) => {
      setPending('text')
      setMessages(data)

      const finalSystemPrompt = configs.systemPrompt || localStorage.getItem('system_prompt') || DEFAULT_SYSTEM_PROMPT
      console.log('🚀 Sending messages with system prompt (first 500 chars):', finalSystemPrompt.substring(0, 500))
      console.log('🚀 Full system prompt length:', finalSystemPrompt.length)
      if (configs.systemPrompt) {
        console.log('✅ Custom system prompt (with brand info) is being used')
      } else {
        console.log('⚠️ Using default system prompt (no brand info)')
      }
        ...
        <ScrollArea className='h-[calc(100vh-45px)]' viewportRef={scrollRef}>
          {messages.length > 0 ? (
            ...
            <motion.div className='flex flex-col h-full p-4 items-start justify-start pt-16 select-none'>
              ...
              >
                <ShinyText text={t('chat:emptyState.title', 'Hello We are prism AI')} />
              </motion.span>
              <motion.span
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <ShinyText text={t('chat:emptyState.subtitle', 'How can I help you today?')} />
              </motion.span>
            </motion.div>
          )}
        </ScrollArea>
