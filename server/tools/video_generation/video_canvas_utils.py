...
async def get_video_info_and_save(
    ...
    if url.startswith("file://"):
        ...
        # Read the local file
        async with aiofiles.open(local_file_path, "rb") as in_file:
            video_content = await in_file.read()
        
        # Determine extension from source file
        source_extension = os.path.splitext(local_file_path)[1][1:]  # Remove dot
        temp_path = f"{file_path_without_extension}.{source_extension}"
        
        # Copy to final location
        async with aiofiles.open(temp_path, "wb") as out_file:
            await out_file.write(video_content)
        print(f"🎥 Video copied to {temp_path}")
        ...
        try:
            ...
            print(f"⚠️ Warning: Could not clean up temporary file {local_file_path}: {e}")
    else:
        # Fetch the video asynchronously from URL
